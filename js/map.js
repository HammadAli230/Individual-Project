...
function renderStats(props) {
  if (!props) {
    statDistance.textContent = statEta.textContent = statCo2.textContent = "—";
    return;
  }
...
function ensureTrafficLayer() {
  if (!TOMTOM_KEY) {
    alert("No TOMTOM_API_KEY set in includes/config.php");
    return null;
  }
  if (trafficLayer) return trafficLayer;
  const url = `https://api.tomtom.com/traffic/map/4/tile/flow/relative0/{z}/{x}/{y}.png?key=${encodeURIComponent(TOMTOM_KEY)}&tileSize=256`;
  console.log("[traffic] using tiles:", url);
...
if (trafficToggle) {
  trafficToggle.addEventListener("change", () => {
    ...
    if (!layer) { trafficToggle.checked = false; return; }
...
async function colorRouteWithTraffic(fc) {
  if (!TOMTOM_KEY || !(trafficToggle?.checked)) return false;
  try {
...
document.getElementById("routeBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    const provider = async () => api("/map/route", { mode, start: s, end: e, traffic });
...
document.getElementById("compareBtn").addEventListener("click", async () => {
  ...
  const e = `${end.getLatLng().lat},${end.getLatLng().lng}`;
  try {
    const provider = async () => api("/map/compare", { start: s, end: e });
    const fc = await provider();
    drawFeatureCollection(fc);
    startTrafficRefresh(provider);
...
document.getElementById("exportBtn").addEventListener("click", async () => {
  ...
  const travel_s    = mins * 60;
  const emissions_g = co2;
  const url = new URL("./api/proxy.php", location.href);
  url.searchParams.set("path", "export/csv");
  const res = await fetch(url, {
    method: "POST",
    body: new URLSearchParams({ mode, length_m, travel_s, emissions_g })
  });
  const txt = await res.text();
  const blob = new Blob([txt], { type: "text/csv" });
  const a = document.createElement("a");
  a.href = URL.createObjectURL(blob);
  a.download = "route_stats.csv";
  a.click();
});
